...
## PRD006 — RPCs transaccionales y flujo de salida de animal

**Milestone:** operaciones de escritura atómicas en Postgres; flujo de salida end-to-end.

### RPCs transaccionales (`20260618000001_rpcs_transaccionales.sql`)

`registrar_compra_animal` — reemplaza las 3 inserciones secuenciales de compra por una única
función `SECURITY DEFINER` que inserta evento + animal + evento_animales en una sola transacción.

`registrar_salida_animal` — flujo de venta y muerte. Un mismo RPC cubre ambos motivos;
la separación semántica vive en los Use Cases, no en la DB.

Helpers privados reutilizables: `_resolve_tipo_evento_id`, `_resolve_motivo_id`,
`_resolve_animal_especie` (prefijo `_`, `STABLE SECURITY DEFINER`).

Patrón documentado en `base_conocimiento/arquitectura/patterns/rpc-transaccional.md`.

### Tipos Supabase generados

`supabase gen types typescript --local > types/supabase.ts` integrado como `npm run db:types`.
Clientes tipados con `createServerClient<Database>` / `createBrowserClient<Database>`.
A partir de PRD006: regenerar tras cada migración que cambie el schema.

### Flujo de salida de animal

Use Cases: `registrarVentaAnimal` y `registrarMuerteAnimal` — mismo RPC, semántica distinta.
AlertDialog de confirmación antes del submit (patrón para acciones irreversibles).
Toast de éxito/error con Sonner. `<Toaster position="top-center" richColors />` en `app/layout.tsx`.

---

## PRD007 — Inicio del módulo reproductivo

**Milestone:** dominio reproductivo operativo end-to-end con patrón CRP validado.

### Patrón CRP implementado

Componentes de dominio en `modules/ganadero/reproductivo/domain/`:
- `ReproductiveContext` — fotografía del dominio antes de aplicar reglas
- `ReproductiveEligibilityRules` — valida elegibilidad (es_reproductora, transición válida)
- `ReproductiveCycleRules` — decide crear vs reutilizar ciclo reproductivo
- `ReproductiveProjection` — construye snapshot (estado_reproductivo + fecha_prevista_parto)
- Tabla `TRANSICIONES` — mapa declarativo de transiciones válidas por evento

### Cubrición end-to-end

Migración `registrar_cubricion` (RPC). Calcula `fecha_prevista_parto`: vacuno +283 días,
porcino +114 días. Abre ciclo reproductivo nuevo o reutiliza si ya existe cubrición en curso.
`FormCubricion` integrado inline en `SeccionAcciones` con animación Framer Motion.
Advertencia contextual cuando el animal ya está en estado `cubierta`.

### Confirmación de gestación end-to-end

Migración `registrar_confirmacion_gestacion` (RPC). No recalcula `fecha_prevista_parto`
(ya calculada por la cubrición). Use Case omite `ReproductiveProjection` — único caso
del pipeline que no necesita Projection.
`FormConfirmacionGestacion` integrado inline en `SeccionAcciones`.

### Mejoras de ficha

- `fecha_prevista_parto` expuesta en `SeccionEstados` con días restantes calculados en UI.
- `EventosList` rediseñado: badge categoría + descripción específica del evento.
  Ordenación `fecha DESC + created_at DESC` como desempate.
- `SeccionAcciones` con visibilidad contextual: botones solo visibles en los estados válidos.

### Fix: estado reproductivo inicial

`registrar_compra_animal` actualizado para inicializar `estado_reproductivo = 'vacia'`
cuando `es_reproductora = true`. Invariante: `es_reproductora = true` ↔ `estado_reproductivo IS NOT NULL`.

### Campo `nombre` en animal

Migración `20260705000000_animal_nombre.sql` — campo opcional para nombre propio del animal.


---

## PRD008 — Consolidación del dominio reproductivo: Confirmación de gestación sin cubrición previa

**Milestone:** dominio reproductivo consolidado — `CONFIRMACION_GESTACION` soporta dos recorridos completos.

### Problema resuelto

Hasta PRD007, `CONFIRMACION_GESTACION` solo era posible con una cubrición registrada previa.
PRD008 elimina esta limitación: en explotaciones extensivas (donde la cubrición no siempre
se registra), la confirmación de gestación puede ser el primer hecho conocido del ciclo.

### Cambios en el dominio

**`domain/rules.ts`** — `TRANSICIONES`:
`CONFIRMACION_GESTACION.desde` ampliado de `['cubierta']` a `['cubierta', 'vacia']`.
La restricción "no Cubrición tras Confirmación" es gratuita: la máquina de estados ya la
garantiza (`gestante` no está en `CUBRICION.desde`).

**`domain/types.ts`**:
- `ReproductiveContext.mesesGestacionEstimados?: number` — dato auxiliar efímero.
  Solo viaja dentro del procesamiento del evento y nunca se persiste.
- `RegistrarConfirmacionGestacionInput.meses_gestacion_estimados?: number` — obligatorio
  desde UI cuando el estado es `vacia`.

**`ReproductiveCycleRules`**:
`CONFIRMACION_GESTACION` adopta el mismo patrón que `CUBRICION`:
`cicloAbierto !== null → reutilizar | cicloAbierto === null → crear`.

**`ReproductiveProjection`** — `buildSnapshot`:
Nuevo caso de cálculo: `CONFIRMACION_GESTACION + mesesGestacionEstimados`:
`fechaPrevistaParto = fecha_confirmacion + (diasGestacion - meses * 30)`.
El pipeline `buildSnapshot` ya es completo para todos los eventos implementados.

**`confirmarGestacion.ts`** (Use Case):
Pasa de pipeline CR a CRP completo. Llama a `buildSnapshot` en ambos recorridos.
Pasa `p_fecha_prevista_parto` al RPC cuando es el primer evento del ciclo.

### RPC actualizado

`registrar_confirmacion_gestacion` v2 (migración `20260802000000`):
- `p_ciclo_id` ahora nullable (antes obligatorio).
- `p_fecha_prevista_parto DATE DEFAULT NULL` — nuevo parámetro.
- Cuando `p_ciclo_id IS NULL`: crea `ciclo_reproductivo` internamente (mismo patrón atómico que `registrar_cubricion`).
- `UPDATE animal SET fecha_prevista_parto = COALESCE(p_fecha_prevista_parto, fecha_prevista_parto)` — solo
  actualiza la fecha cuando se proporciona; preserva la existente cuando viene de cubrición.

### Cambios de UI

**`SeccionAcciones`**: botón "Confirmar gestación" visible cuando
`estado_reproductivo ∈ {cubierta, vacia}` (antes solo `cubierta`).

**`FormConfirmacionGestacion`**: formulario extendido condicionalmente cuando no hay cubrición previa:
- Bloque de aviso informativo (implicaciones de la acción).
- Selector de meses 1–9 (estimación de edad gestacional). La interfaz trabaja
  exclusivamente con meses completos; internamente se usa para calcular días restantes.
- El campo de meses es obligatorio en este recorrido y se valida en `onSubmit`.

### Evals

`evals/confirmacion-gestacion.eval.ts` — 14 casos cubriendo:
transiciones de estado, elegibilidad, decisión de ciclo y proyección para ambos recorridos.
