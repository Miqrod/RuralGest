-- =============================================================================
-- registrar_destete: destete atómico de una cría
--
-- Operaciones en una sola transacción:
--   1. Valida que la cría es elegible (Cría, viva, vínculo activo)
--   2. Actualiza la cría: tipo_productivo → Recría, vínculo → finalizado
--   3. Crea el evento DESTETE ligado al ciclo de la cría
--   4. Registra madre y cría en evento_animales
--   5. Evalúa cierre de ciclo: si no quedan vínculos activos, cierra el ciclo
--      y devuelve a la madre a estado_reproductivo = 'vacia'
--
-- El nuevo ciclo vacío para reproductoras (es_reproductora = true) se crea
-- en registrar_nuevo_ciclo_tras_destete — separado para respetar SRP.
--
-- Ref: PRD010 §3, §O23, §O31
-- =============================================================================
...
BEGIN
  ...
  IF v_ciclo_id IS NOT NULL THEN
    ...
    IF v_active_bonds = 0 THEN
      ...
      -- La madre vuelve a 'vacia' al terminar la lactancia
      IF v_cria.madre_id IS NOT NULL THEN
        UPDATE animal
        SET estado_reproductivo = 'vacia'
        WHERE id = v_cria.madre_id;
      END IF;
